...
**Active schema pack (GPT-SSR-03 / GIM-265…275):** before stash mapping, **read** [`schema-packs/README.md`](schema-packs/README.md) for the active `schema_id` / `schema_version` pair, then open the two files named there. **MUST** emit matching `schema_binding` on every `StoryDraftStashRequest`. Fill `structured_payload` from that pack’s data-model **and** `normalized_issue_payload.structured_payload_handoff` (normalizer §4.7) — no invent keys or enum values. Apply pack `geo_intake.mode` to geo **emit and pre-flight** (`optional` / `require_location_or_detail` / `require_detail` — full matrix in the pack). Do **not** hard-code civic field names or civic pack paths in this core file. Envelope `schema_version` stays `m2.story_intake_envelope.v2`. PII / admission process unchanged.
...
| `schema_binding.structured_payload` | `normalized_issue_payload.structured_payload_handoff` (normalizer §4.7) / active data-model pack §2 | Yes | Object. Keys ⊆ pack payload (`signals.*`, optional `geo.*`). Fill pack-required signal fields from the handoff when the pack lists them required. Empty/missing pack-required keys → item 20 STOP. No invent OpenAPI tokens or enum values. Narrative-first: structured is **additional** (AC-GPT-REQ3-04). |

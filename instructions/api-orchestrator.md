...
**Active schema pack (GPT-SSR-03 / GIM-265…275; GPT-SSR-09 capstone):** before stash mapping, **read** [`schema-packs/README.md`](schema-packs/README.md) for the active `schema_id` / `schema_version` pair, then open the pack artifacts named there: **`pack.json`**, **`payload.schema.json`**, **`taxonomy.json`**, **`inbound-validation.md`**, **`interview-overlay.md`**. **MUST** emit matching `schema_binding` on every `StoryDraftStashRequest`. Fill `structured_payload` from `pack.json` `field_policy` + `normalized_issue_payload.structured_payload_handoff` (normalizer §4.7) — no invent keys or enum values. Read `geo_intake` from **`pack.json`** (not archived MD). Apply pack `geo_intake.mode` to geo **emit and pre-flight**. Do **not** hard-code civic field names in this core file. Envelope `schema_version` stays `m2.story_intake_envelope.v2`. PII / admission process unchanged.

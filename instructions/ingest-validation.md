...
**REQ-22 wire handoff:** Story Intake **user** transport is **stash** via `postStoryDraftStash` (`POST /story-drafts`, `m2.story_intake_envelope.v2`) per [`api-orchestrator.md`](api-orchestrator.md) §5.2 — not direct `POST /intake/stories` from GPT Actions (service seed/sim only). Validation must preserve language signals for [`story-normalizer.md`](story-normalizer.md): `session_language` (UI primary) and `detected_input_language` (user narrative text). Do not block strict-chain progression solely because Issue draft routes are unused.

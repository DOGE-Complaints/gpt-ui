...
**REQ-22 wire handoff:** Story Intake transport is **`POST /intake/stories`** with `m2.story_intake_envelope.v2` only ([`api-orchestrator.md`](api-orchestrator.md) §5.2.1). Validation must preserve language signals for [`story-normalizer.md`](story-normalizer.md): `session_language` (UI primary) and `detected_input_language` (user narrative text). Do not block strict-chain progression solely because Issue draft routes are unused.
